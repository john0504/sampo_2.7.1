...
@Component({
  ...
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => BarChart), multi: true }],
  encapsulation: ViewEncapsulation.None,
})
export class BarChart extends UIComponentBase implements OnInit, OnDestroy {
  title: string = '';
  ...
  constructor(
    ...
    super(ims, 'bar-chart', null);
    this._barChartModel = new BarChartModel();
    this._barChartModel.valueTextCallback = this._modelCallback;
    this._options = cloneDeep(this._barChartModel.basicModel);
    this._ims = ims;
    this.logics = [];
    this.states = [];
  ...
  private processResults(e, results?) {
    ...
    results.forEach((historicalData, index) => {
      ...
      const avg = this._noData ? -32767 : (sum / count) | 0;
      const avgText = this._modelCallback(avg, index);
      const g = this._barChartModel.averageTag(avgText);
