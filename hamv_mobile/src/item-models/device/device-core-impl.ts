...
export class DeviceCoreImpl implements DeviceCore {
  ...
  private updateChartComponents(uiModel) {
    ...
    if (chartLayout) {
      ...
      if (main) {
        main.forEach((name) => {
          const m = this.sanitizeComponent(uiModel.chartComponents[name]);
          if (m) {
    ...
    if (controlLayout) {
      if (controlLayout.primary) {
        controlLayout.primary.forEach((name) => {
          const m = this.sanitizeComponent(uiModel.components[name]);
  ...
  private sanitizeComponent(component) {
    if (!component) return;
    const isAllModelKeysInStatus = component.models.every(model => this.device.status[model.key] != null);
    return isAllModelKeysInStatus ? component : false;
  }
}
