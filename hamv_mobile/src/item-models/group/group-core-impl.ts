...
export class GroupCoreImpl implements GroupCore {
  ...
  private generateControls(possibleModelNames = []) {
    const uiModels = possibleModelNames
      .map(modelName => this.ims.getUIModelFromName(modelName))
      .filter(Boolean);

    if (!uiModels.length) return [];
  ...
  public selfUpdate() {
    ...
    if (hasDevices) {
      ...
      if (!isEqual(possibleModelsNames, this._possibleModelsNames)) {
        ...
        this._controllers = this.filterByFieldsRange(this._devices, this._controllers);
      }
  ...
  private filterByFieldsRange(devices = [], controllers = []) {
    const mappings = devices
      .map(device => device.fields_range
        .reduce((o, x) => Object.assign(o, x), {})
      );
    const mergedMapping = mappings.reduce((o, x) => Object.assign(o, x), {});
    this.fields_range = [mergedMapping];
    const differentKeys = Object.keys(mergedMapping)
      .filter(key => mappings
        .some(x => x[key] !== mappings[0][key])
      );
    return controllers
      .filter(c => c.models
        .every(m => !differentKeys.includes(m.key))
      );
  }
}
